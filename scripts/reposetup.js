...
try {
    ...
    MODULES.forEach(module => {
        ...
        if (fs.existsSync(modulePath)) {
            ...
            execSync("npm link", { cwd: modulePath, stdio: "inherit" });
        } else {
            console.error(`Module ${module} not found in ${CLONE_DIR}`);
        }
    });

    // Link the modules in the main project
    MODULES.forEach(module => {
        console.log(`Using linked module: ${module}`);
        execSync(`npm link @lionweb/repository-${module}`, { stdio: "inherit" });
    });
