import { TreeNodeData } from "$lib/components/tree/TreeNodeData.js"
...
import { isFromClient, isToClient } from "../../../deltaclients/monitor.svelte.js"; // adjust path

// -------------------- public API --------------------

export function deltaEventToTreeNodeData(ev: MessageFromClient | MessageToClient): TreeNodeData {
    const common: TreeNodeData[] = isToClient(ev) ? [
        leaf("kind", ev.messageKind),
...
function eventSpecificChildren(ev: MessageFromClient | MessageToClient): TreeNodeData[] {
    switch (ev.messageKind) {
        ...
        default:
            ...
            return [valueNode(`default ${ev.messageKind}`, ev)];
    }
}
